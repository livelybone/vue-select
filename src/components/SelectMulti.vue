<template v-if="valid">
  <div class="select-multi" :class="{ disabled: !canEdit }" ref="wrap">
    <div class="value values">
      <div v-for="(o, i) in selected" class="val" :key="i">
        <span class="v" v-html="o.name"></span>
        <span v-if="canEdit" class="icon-del" @click.stop="click(o)"></span>
      </div>
      <template v-if="canEdit">
        <input
          ...
          ref="input"
        />
        <span v-else-if="selected.length <= 0" class="val placeholder">{{
          $placeholder
        }}</span>
      </template>
    </div>
...
export default {
  ...
  computed: {
    selected() {
      return this.mergedOptions.filter(op =>
        this.value.some(v => v === op.value),
      )
    },
    showOptions() {
  ...
  methods: {
    html(o) {
      return `${o.name}${
        this.isSelected(o.value) ? '<span class="icon-selected"></span>' : ''
      }`
    },
    click({ value }) {
      if (this.canEdit) {
