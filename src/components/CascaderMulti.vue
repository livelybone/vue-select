<template v-if="valid">
  <div class="cascader-multi" :class="{ disabled: !canEdit }" ref="wrap">
    <div class="value values">
      ...
      <template v-if="canEdit">
        <input
          ...
          ref="input"
        />
        <span v-else-if="showSelected.length <= 0" class="val placeholder">{{
          $placeholder
        }}</span>
      </template>
    </div>
...
export default {
    ...
    showSelected() {
      ...
      return selected.map(s =>
        s.reduce(
          (pre, item) => {
            if (pre.name)
              pre.name += ` <span class="split">/</span> ${item.name}`
  ...
  methods: {
    ...
    html(o, selected, isEnd) {
      return `${o.name}${
        selected && isEnd ? '<span class="icon-selected"></span>' : ''
      }`
    },
    setSelect(op, index) {
